...
def calculate_level(exp: int) -> int:
    ...
    while True:
        next_level = level + 1
        exp_for_next = calculate_exp_for_level(next_level)
        
        if total_exp_required + exp_for_next > exp:
            break
            
        total_exp_required += exp_for_next
        level = next_level
...
def update_exp(server_id, user_id, exp: int) -> Tuple[int, int, bool]:
    """Update user experience and return level information"""
    user_data = get_user_data(server_id, user_id)
    old_level = calculate_level(user_data["exp"])
    
    # Update the exp
    user_data["exp"] = max(0, exp)
    
    # Calculate new level based on updated exp
    new_level = calculate_level(user_data["exp"])
    
    # Update the level in user data
    user_data["level"] = new_level
    
    # Save changes
    save_data(server_data)
    
    # Return old level, new level, and whether leveled up
