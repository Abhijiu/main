...
async def profile(interaction: discord.Interaction, member: discord.Member = None):
    try:
        ...
        embed.add_field(name="Level", value=f"<a:arrow_arrow:1344705810516676608> {level}", inline=False)
        embed.add_field(name="Currency", value=f"<a:arrow_arrow:1344705810516676608> {user.get('currency', 0)} <a:Rubles:1344705820222292011>", inline=False)
        embed.add_field(
            name="Progress to Next Level",
            value=f"<a:arrow_arrow:1344705810516676608> {progress:.1f}% ({exp:,}/{next_level_exp:,} EXP)",
            inline=False
        )
...
async def pull(interaction: discord.Interaction, amount: int = 1):
    try:
        class InteractionContext:
            ...
            async def _send(self, *args, **kwargs):
                # Handle both regular and ephemeral messages
                if kwargs.get('ephemeral', False):
                    return await self.interaction.followup.send(*args, **kwargs)
                else:
        ...
        # Call pulls_command from bounty.py
        await pulls_command(ctx, amount)

    except BountyCommandError as bounty_error:
        # Handle bounty-specific errors
        await interaction.followup.send(
            f"<a:Animated_Cross:1344705833627549748> {str(bounty_error)}", 
            ephemeral=True
        )
        logger.error(f"Bounty error in pull command: {bounty_error}")
    except Exception as e:
        ...
        logger.error(f"Unexpected error in pull command: {e}", exc_info=True)
